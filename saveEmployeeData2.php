	 ...
	 if (mysqli_num_rows($result) == 1) {
		...
		header('Location: ' . $home_url);
	 	...
	 	if(isset($_REQUEST['fullname']) && isset($_REQUEST['empid']) && isset($_REQUEST['email']) && isset($_REQUEST['password'])){
			...
			$query = "insert into employees (fullname,empid,email,password1,radioText,projectName,projectManager,exp) values ('" . $_REQUEST["fullname"] . "','" . $_REQUEST["empid"] . "', '" . $_REQUEST["email"] . "','" . $_REQUEST["password"] . "','N','" . $_REQUEST["projectName"] . "','" . $_REQUEST["projectManager"] . "','" . $_REQUEST["exp"] . "')";
			...
			$_SESSION['radioText'] = 'N';
			$_SESSION['exp'] = $_REQUEST['exp'];
			$_SESSION['projectName'] = $_REQUEST['projectName'];
			$_SESSION['projectManager'] = $_REQUEST['projectManager'];
			$result = mysqli_query($db_server, $query);
